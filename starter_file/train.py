...
def clean_data(data):
    ...
    X_train = data.dropna(how='any',axis=0)
        
    # one hot encoding using pandas method (get_dummies)
    Gen = pd.get_dummies(X_train.Gender, prefix='Gender')
    X_train.drop("Gender", inplace=True, axis=1)
    X_train = X_train.join(Gen)
    X_train = X_train[['Age', 'Gender_Female', 'Gender_Male', 'T_Bili', 'D_Bili', 'AA_Phosphate', 'SgptA_Aminotrans', 'SgotA_Aminotrans','T_proteins', 'ALB_Albumin', 'AG_AlbuminR_GlobulinR','Result']]
